...
- [ ] **0.2** Implement `cityFileBytes`/`tileWords` read support and canonical write-to-`cityFileBytes` behavior.
- [ ] **0.3** Add validation for mutual exclusivity of map forms and key namespace conventions (`builtin/*`, `user/*`).
...
### Requirements (Task Checklist)
- [ ] **1.1** Create `packages/scenario-runtime` with declarative event/objective runtime state.
- [ ] **1.2** Replace hardcoded `ScenarioID` conditionals in sim runtime paths with scenario-runtime inputs.
- [ ] **1.3** Port classic built-in scenarios to declarative data (`builtin/*`) with parity behavior.
...
### Requirements (Task Checklist)
- [ ] **2.1** Move scenario selection and runtime protocol to `scenarioKey`-based flows.
- [ ] **2.2** Add `Load Scenario...` file picker in Micropolis menu for external scenario JSON.
- [ ] **2.3** Add pre-start review dialog after loading scenario (metadata summary + difficulty selection + explicit start).
