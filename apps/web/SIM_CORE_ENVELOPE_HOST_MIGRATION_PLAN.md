...
- [x] New host has no dependency on `buildInitialDemoMapTiles` or demo custom placement functions.
...
- [x] Snapshot + replay reconstruct map/HUD/messages/realtime deterministically.
- [x] Realtime overlays remain functional with snapshot + delta transport.
...
- [x] Disaster-triggered message/realtime payload behavior remains C-parity-aligned.

### Phase 5: Route Cutover + Demo Host Deletion

- [x] Change `createPlayableRuntimeHost()` to return the new sim-core envelope host.
- [x] Remove `DemoMapHost` imports from gameplay-path modules. (Revalidated on 2026-02-10.)
