...
- `ref/micropolis/spec/persistence/SPEC.md`:
  ...
  - Harness-level command equivalence coverage now verifies that `load-cty --cty-path` and `load-cty-bytes` produce identical loaded oracle state for the same valid payload (`packages/micropolis-c-harness/src/core-parity.test.ts`), and non-I/O parity callsites in sim-core use bytes-based loader plumbing (`packages/sim-core/src/io/cty.test.ts`).
