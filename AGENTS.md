- if you edited any code files (.md does not count), make sure to run `pnpm typecheck`, `pnpm lint`, and `pnpm format` when you're done. don't run them after every single step, just before you cede control back to me.
