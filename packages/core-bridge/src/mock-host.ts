...
/**
 * Construction options for `MockHost`.
 * Mirrors command ingress and version-lockstep expectations from
 * `ref/micropolis/src/sim/w_sim.c` and packet/error fanout intent from
 * `ref/micropolis/src/sim/w_net.c`.
 * Parity note: this is intentionally a deterministic in-memory bridge host,
 * not a 1:1 transport/socket implementation.
 */
export interface MockHostOptions extends MockAuthorityEngineOptions {
  protocolVersion?: string;
  coreVersion?: string;
}

/**
 * Deterministic `CoreHost` adapter over `MockAuthorityEngine`.
 * Mirrors authoritative command intake shape from `SimCmd` in
 * `ref/micropolis/src/sim/w_sim.c` and ordered outbound event fanout intent
 * from `ref/micropolis/src/sim/w_net.c`.
 * Parity note: unlike Micropolis C command/IO coupling, this host is a typed
 * event emitter used for bridge contract conformance testing.
 */
export class MockHost implements CoreHost {
