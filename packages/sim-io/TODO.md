...
- [x] Harness negative-path command parity coverage for invalid `.cty` payloads (`load-cty --cty-path` vs `load-cty-bytes`) including failure exit-code parity and unchanged-state assertions (`packages/micropolis-c-harness/src/core-parity.test.ts`)
- [x] Reusable non-throwing harness failure-probe wrappers for `.cty` loads (`runCoreOracleLoadCtyFailureProbe`, `runCoreOracleLoadCtyBytesFailureProbe`) exposing `exitStatus`/`signal`/`stderr` plus pre/post `save-cty` bytes for unchanged-state checks (`packages/micropolis-c-harness/src/core-parity.ts`)
